...
controller.list = (req, res) => {
  req.getConnection((err, conexi) => { 
    conexi.query('SELECT * FROM authors;', (err, rows, fields) => { // rows es un array
      if (err) {
        res.json(err);
      } else {
...
controller.save = (req, res) => {
  const data = req.body;
  req.getConnection((err, conexi) => { 
    conexi.query('INSERT INTO authors (name, nacionality) VALUES (?, ?);',
      [data.name, data.nacionality], (err, rows) => { // rows es un array
      if (err) {
        res.json(err);
      } else {
        res.redirect('/');
...
controller.delete = (req, res) => {
  ...
  req.getConnection((err, conexi) => { 
    conexi.query('DELETE FROM authors WHERE author_id = ?',
      [id], (err, rows) => { // rows es un array
      if (err) {
        res.json(err);
      } else {
        res.redirect('/');
...
controller.edit = (req, res) => {
  const { id } = req.params; // desestructurar el objeto
  req.getConnection((err, conexi) => { 
    conexi.query('SELECT * FROM authors WHERE author_id = ?',
      [id], (err, rows) => { // rows es un array
      if (err) {
        res.json(err);
      } else {
        res.render('author_edit', { // renderiza authors.ejs
...
controller.editUpdate = (req, res) => {
  ...
  req.getConnection((err, conexi) => { 
    conexi.query('UPDATE authors SET name = ?, nacionality = ? WHERE author_id = ?',
      [name, nacionality, id], (err, rows) => { 
      if (err) {
        res.json(err);
      } else {
        res.redirect('/');
